...
const ChainlinkAllowanceModal: React.FC<MintNftModalProps> = ({ isOpen, onClose }) => {
  ...
  const { isLoading } = useWaitForTransaction({
    ...
    onSuccess() {
      toast({
        title: 'Success',
        description: `Successfully approved Chainlink allowance.`,
        status: 'success',
      });
      onClose();
    },
  ...
  const handleApprove = ({ value }: FormValues) => {
    if (account?.address) {
      write({
        args: [nftContractAddress, BigInt(value)],
      });
    }
  };
    ...
    <>
      <Modal isOpen={isOpen} onClose={onClose} size="lg">
        ...
        <form onSubmit={handleSubmit(handleApprove)}>
          <ModalContent mx={3}>
            <ModalHeader>Change Chainlink Allowance</ModalHeader>
            <ModalCloseButton />
            <ModalBody pb={6}>
              <Stack spacing={3}>
                <Stack direction={'row'}>
...
export default ChainlinkAllowanceModal;
